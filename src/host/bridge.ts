...
export class SessionBridge {
  ...
  private pendingPolicy = new Map<string, { mode: string; at: number }>()
  ...
  private async runCommand(msg: InboundMessage, text: string): Promise<string> {
    ...
    switch (cmd) {
      ...
      case '/stop':
        return await this.stopSession(msg.windowKey)
      case '/new':
        return await this.newSession(msg.windowKey, arg)
      case '/policy':
        return this.policyCommand(msg.windowKey, arg)
      default:
  ...
  private showStatus(windowKey: string): string {
    ...
    if (agent?.options?.model) {
      lines.push(`模型：${agent.options.provider ? agent.options.provider + ' / ' : ''}${agent.options.model}`)
    }
    if (agent?.session) {
      try {
        const mode = this.ctx.sandboxPolicy?.resolve?.({ session: agent.session })?.mode
        const labels: Record<string, string> = {
          'read-only': '只读', 'workspace-write': '工作区写', 'danger-full-access': '完全开放',
        }
  ...
  private async stopSession(windowKey: string): Promise<string> {
    const sessionId = this.auth.getBinding(windowKey)?.sessionId
    if (!sessionId) return '未绑定会话。回复 /sessions + /use <编号> 先绑定。'
    ...
    try {
      ...
      this.turnStatus.set(sessionId, 'idle')
      this.auth.audit('session/cancel', { sessionId, windowKey })
      return '🛑 已终止当前任务。'
    } catch (error: any) {
   ...
   */
  private policyCommand(windowKey: string, arg: string): string {
    const sessionId = this.auth.getBinding(windowKey)?.sessionId
    if (!sessionId) return '未绑定会话。回复 /sessions + /use <编号> 先绑定。'
    const agent = this.liveAgentOf(sessionId)
    ...
    if (mode === 'danger-full-access') {
      this.pendingPolicy.set(windowKey, { mode, at: Date.now() })
      return '⚠️ 完全开放将允许任意文件读写，回复 YES 确认（60 秒内有效）。'
    }
    return this.applyPolicy(agent.session, mode, windowKey)
  }

  private applyPolicy(session: any, mode: string, windowKey: string): string {
    const labels: Record<string, string> = {
      'read-only': '只读', 'workspace-write': '工作区写', 'danger-full-access': '完全开放',
    }
    try {
      // Identical to dsh-sandbox-policy's setSandboxMode(): append the fold event.
      session.append('sandbox/mode', { mode })
      this.auth.audit('policy/set', { sessionId: session.id, mode, windowKey })
   ...
   */
  private async newSession(windowKey: string, prompt: string): Promise<string> {
    const api = this.apiProxy()
    if (!api?.sessions?.create) return '⏳ 当前环境不支持创建会话（apiProxy 不可用）。'
    const boundId = this.auth.getBinding(windowKey)?.sessionId
    const cwd =
      (boundId ? this.liveAgentOf(boundId)?.session?.header?.cwd : null) ??
      this.lastActiveRoot?.session?.header?.cwd ??
      this.roots()[this.roots().length - 1]?.session?.header?.cwd
    ...
    this.auth.audit('session/create', { windowKey, sessionId: newId, cwd, hadPrompt: Boolean(prompt) })

    if (!prompt) return `✅ 已在当前工作区创建并绑定新会话（${shortId(newId)}）。直接发消息即开始。`

    // Deliver the initial prompt: the agent may register a beat after create.
    let agent: any = null
