	...
	*/
	...
	registryRank() {
		...
		try {
			...
			const order = raw?.global?.workspaceIds ?? Object.keys(workspaces);
			let n = 0;
			for (const wid of order) for (const sid of workspaces[wid]?.sessionIds ?? []) rank.set(sid, n++);
		} catch {}
		return rank;
	}
	/** The GUI-identical catalog: same source, same filters, same ordering. */
	async allSessionsViaApi(api, includeChildren) {
	...
	async allSessionsLegacy(includeChildren = false) {
		const out = [];
	...
	*/
	async sendFileForSession(sessionId, relPath, caption) {
		...
		if ("error" in resolved) {
			this.auth.audit("file/send-failed", {
				sessionId,
				relPath,
		...
		if (windows.length === 0) {
			this.auth.audit("file/send-failed", {
				sessionId,
				relPath,
				reason: "no-bound-window"
			});
			return "发送失败：该会话没有绑定任何 IM 窗口（在微信/飞书里 /use 绑定后再试）。";
		}
		let sent = 0;
		let lastError = null;
		for (const windowKey of windows) try {
...
*/
function saveProfileOverride(current, next, logger) {
	const dshHome = process.env.DSH_HOME ?? join(homedir(), ".dsh");
	const patchPath = typeof current.profilePatch === "string" && current.profilePatch || join(dshHome, "profiles", "web", "cordis.patch.yml");
	const merged = {
		...current,
		...next
	};
	let rows = [];
	if (existsSync(patchPath)) try {
	...
	writeFileSync(patchPath, yaml.dump(rows, {
		noRefs: true,
		lineWidth: 120
	}), "utf8");
